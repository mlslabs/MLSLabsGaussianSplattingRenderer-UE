...
<p align="center">
  ...
  <img alt="UE Version" src="https://img.shields.io/badge/Unreal_Engine-5.8-white?logo=unrealengine&logoColor=white&color=0E1128">
...
[**教学文档**](https://github.com/mlslabs/MLSLabsGaussianSplattingRenderer-UE/blob/Pro_2.0_ue5.8/docs/README_CN.md) •
[**入门指南**](#入门指南) •
[**安装**](#installation) •
...
[**版本记录**](#版本记录) •
[**许可证**](https://github.com/mlslabs/MLSLabsGaussianSplattingRenderer-UE/blob/Pro_2.0_ue5.8/LICENSE)
...
- **高压缩比静态3DGS：** 支持用于高压缩比3DGS 的 `.sog` 格式。
...
2. **关于项目打包：** 为确保项目顺利打包，请将 `MLSLabsRenderer` 文件夹复制到 UE5.8 引擎安装目录中（例如：`Epic Games\UE_5.8\Engine\Plugins\Marketplace`）。
...
2. 路径管理： 实现 PLY 数据导入后的自动复制与相对路径引用，优化项目分发体验。
3. 编辑器优化： 修复高斯数据加载后的包围盒计算逻辑，确保编辑器 Gizmo 坐标显示准确。
4. 兼容性维护： 完善针对 Unreal Engine 5.7 的打包脚本，确保 .ply 与 .sog 资产能正确被包含在 Release 包中。

**Lite_V1.0.0.10_beta**
1. 修复了 Scaled Gaussian Splatting 节点颜色显示异常的问题。
2. 解决了打包过程中删除库文件（如 cublas64_12.dll）时出现的“访问被拒”错误。
3. 修复了当 Pitch、Yaw、Roll（俯仰、偏航、翻滚）同时操作时，高斯角色旋转不正确的问题。
4. 新增对多显卡系统中非主显卡（ID > 0）渲染的支持。
5. 支持拷贝导入的 PLY 数据并使用相对路径引用，以确保无缝打包与分发。
6. 加载高斯数据后将自动更新并计算包围盒（Bounding Box），以确保编辑器中的坐标轴（Gizmo）显示正确。
